function targets = classify_parametric(param_struct, patterns)

%Function for classifying patterns based on parametric distributions.
%Inputs are the parameters of the distributions and the patterns.
...
for j = 1:Nc,
    ...
    for i = 1:length(param_struct(j).w),
        ...
        if strcmp(type, 'Gaussian') 
            ...
            if (Dim == 2)
                   invsigma = inv(sigma);
                   V(j,:) = V(j,:) + p*w ./ (2 * pi * sqrt(abs(det(sigma)))) .* ...
